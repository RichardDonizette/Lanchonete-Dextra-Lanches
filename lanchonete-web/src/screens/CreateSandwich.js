import React, { Component } from 'react';
import { connect } from 'react-redux';

import Typography from '@material-ui/core/Typography';
import Card from '@material-ui/core/Card';
import CardContent from '@material-ui/core/CardContent';
import Grid from '@material-ui/core/Grid';
import Divider from '@material-ui/core/Divider';
import Button from '@material-ui/core/Button';
import Icon from '@material-ui/core/Icon';
import burgerImg from '../../src/assets/img/x-bacon.png';

import Header from '../components/Header'

class CreateSandwich extends Component {
    render() {
        return (
            <div>
                <Header description={'Monte seu sanduíche como quiser'} />
                <Card>
                    <CardContent>
                        <Typography variant="display1" style={{ fontWeight: 200 }}>
                            Monte seu <span style={{ color: "#f50057", fontWeight: "bold" }}>delicioso</span> sanduíche!
              </Typography>
                        <Grid container spacing={8} style={{ marginTop: 10, marginBottom: 10 }}>
                            <Grid item xs={12} style={{ textAlign: "left" }}>
                                <Typography variant="subheading">
                                    Seus sanduíches
                      </Typography>
                            </Grid>
                            <Grid item xs={12} >
                                <div>
                                    <div style={{ textAlign: "left", whiteSpace: "nowrap", overflowX: "scroll", width: "100%" }}>
                                        <Button color="secondary" onClick={() => { }} style={{ "border": "1px #d2d2d2 solid", "width": "70px", "height": "70px", borderRadius: 4, background: "#f50057", marginRight: 10, marginBottom: 10 }} >
                                            <img src={burgerImg} alt="Avatar" className={"image"} style={{ width: '200%', height: '200%', borderRadius: 10 }} />
                                        </Button>

                                        <Button onClick={() => { }} style={{ "border": "1px #d2d2d2 solid", "width": "70px", "height": "70px", borderRadius: 4, background: "#f50057", marginRight: 10, marginBottom: 10 }} >
                                            <Icon>add_circle</Icon>
                                        </Button>
                                    </div>
                                    <Button color="secondary" disabled={true} onClick={() => { }} variant="contained" style={{ marginTop: 10 }} >
                                        {"Adicionar ao carrinho este sanduíche"}
                                    </Button>
                                </div>
                            </Grid>

                        </Grid>
                        <Grid container spacing={8} style={{ marginTop: 10, marginBottom: 10 }}>
                            <Grid item xs={6} style={{ textAlign: "left" }}>
                                <Typography variant="subheading">
                                    Ingredientes:
                      </Typography>
                            </Grid>
                            <Grid item xs={6} style={{ textAlign: "right" }}>
                                <Typography variant="display1">
                                    "R$0,00"
                                </Typography>
                            </Grid>
                            <Grid item xs={12} style={{ textAlign: "left" }}>
                                <Typography variant="h6">
                                    proteins
                                </Typography>
                                <Typography variant="h6">
                                    salads
                                </Typography>

                            </Grid>
                        </Grid>

                        <Divider />

                        <Grid container spacing={8} style={{ marginTop: 10 }}>
                            <Grid item xs={6}>
                                <Typography variant="h6">
                                    Escolha suas proteinas
                      </Typography>
                                listproteinas
                            </Grid>
                            <Grid item xs={6}>
                                <Typography variant="h6">
                                    Escolha sua salada
                      </Typography>
                                list salada
                            </Grid>
                        </Grid>
                    </CardContent>
                </Card>

            </div>
        )
    }
}
const mapStateToProps = (state) => ({
});

export default connect(mapStateToProps)(CreateSandwich);